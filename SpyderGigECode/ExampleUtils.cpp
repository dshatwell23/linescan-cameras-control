...
#ifndef SAP_CAMERA_SDK
bool GetParametersFrom3DBuffer(SapBuffer *Buffer, int* Invalid_Flag_C,
   double* Invalid_Value_C, double* Scale_A, double* Offset_A, double* Scale_C,
   double* Offset_C, BUFFER_LAYOUT* Buffer_Layout, int* Output_Mode)
{
	...
	if (!Buffer->GetParameter(CORBUFFER_PRM_SCAN3D_INVALID_DATA_FLAG_C, Invalid_Flag_C))
		return false;
	if (!Buffer->GetParameter(CORBUFFER_PRM_SCAN3D_COORD_SCALE_A, Scale_A))
		return false;
	if (!Buffer->GetParameter(CORBUFFER_PRM_SCAN3D_COORD_OFFSET_A, Offset_A))
		return false;
	...
	if (!Buffer->GetParameter(CORBUFFER_PRM_SCAN3D_COORD_OFFSET_C, Offset_C))
		return false;
	if (!Buffer->GetParameter(CORBUFFER_PRM_SCAN3D_OUTPUT_MODE, Output_Mode))
		return false;
